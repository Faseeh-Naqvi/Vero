...
export default function RebookSlots({ patient, originalAppointment, variant = 'default', onReplacementBooked }) {
  const {
    physicians,
    appointments,
    addAppointment,
  ...
  const handleConfirm = () => {
    ...
    if (originalAppointment?.id) {
      updateAppointment(originalAppointment.id, { replacedByAppointmentId: newAppt.id });
    }
    setPickedSlot(null);
